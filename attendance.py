...
# Second Image
        ...
        img1=img1.resize((683,200),Image.Resampling.LANCZOS)
...
# bg img
        ...
        main_frame.place(x=0,y=50,width=1366,height=480)
        
        # left label frame
        Left_frame=LabelFrame(main_frame,bd=2,bg="white",relief=RIDGE,text="Student attendance Details",font=("times new roman",12,"bold"))
        Left_frame.place(x=10,y=10,width=680,height=410)
        
        img_left=Image.open(r"college_images\AdobeStock_303989091.jpeg")
        img_left=img_left.resize((670,130),Image.Resampling.LANCZOS)
        ...
        left_inside_frame.place(x=0,y=135,width=670,height=250)
        ...
        Right_frame.place(x=695,y=10,width=650,height=410)
        
        table_frame=Frame(Right_frame,bd=2,relief=RIDGE,bg="white")
        table_frame.place(x=5,y=5,width=630,height=370)
    ...
    #====================scroll  bar table======================
        ...
        self.AttendanceReportTable.column("id",width=87)
        self.AttendanceReportTable.column("roll",width=87)
        self.AttendanceReportTable.column("name",width=87)
        self.AttendanceReportTable.column("department",width=87)
        self.AttendanceReportTable.column("time",width=87)
        self.AttendanceReportTable.column("date",width=87)
        self.AttendanceReportTable.column("attendance",width=87)
    ...
    def exportCsv(self):
      # print(mydata)
      try:
          if len(mydata) < 1:  # Assuming mydata is a class attribute
            messagebox.showerror("No Data", "No Data found to export", parent=self.root)
            return False

          fln = filedialog.asksaveasfilename(initialdir=os.getcwd(), title="Save CSV",defaultextension=".csv",
                                           filetypes=(("CSV File", "I*.csv"), ("All File", "*.*")), parent=self.root)

          if not fln:  # Check if file path is not empty
            return False
